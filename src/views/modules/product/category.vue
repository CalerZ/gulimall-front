...
export default {
  ...
  methods: {
    ...
    deleteLists() {
      let catid = [];
      let catname=[]
      let delets = this.$refs.metuTree.getCheckedNodes();
      for (let i = 0; i < delets.length; i++) {
        catid.push(delets[i].catId);
        catname.push(delets[i].name)
      }
